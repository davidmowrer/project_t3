...
feature 'User Authentication' do
  scenario 'allows a users to signup' do
    ...
    fill_in 'Email', with: 'david.jones@gmail.com'
    fill_in 'Password', with: 'sup3rs3krit'
    fill_in 'Password Confirmation', with: 'sup3rs3krit'

    click_button 'Signup'

    expect(page).to have_text('Thank you for signing up David')
    expect(page).to have_text('Signed in as david.jones@gmail.com')
  end
  ...
  scenario 'allows existing users to logout' do
    user = FactoryGirl.create(:user, password: 'sup3rs3krit')
